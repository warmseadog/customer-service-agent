...
核心流程：
  ...
  4. 内部升级收件人：仅 **产品 owner_email** → **fallback_owner_email**；未配置则不发送内部通知（不使用仪表盘/.env 全局兜底）
...
def _get_calm_internal_recipient(
    ...
    """
    安抚类内部通知收件人：与是否激烈无关，仅产品 owner / fallback 链。
    ``intense`` 保留参数供调用方签名稳定，不影响路由结果。
    """
    del intense
...
def _handle_one_email(
    ...
    *,
    ...
    # 语气 hostile 视为需要「跪舔级」安抚；内部通知仍仅走产品负责人链
    ...
    def _do_send_internal(reason: str, for_calm: bool) -> bool:
        ...
        try:
            if not owner_email:
                logger.warning("⚠️ 升级无收件人：产品未配置负责人或备用负责人邮箱，已跳过内部通知")
                return False
            # 避免将「客服工单」误发到客户邮箱（产品负责人若误填为客户邮箱）
            ...
            if _owner_l and (_owner_l == _from_l or (_contact_l and _owner_l == _contact_l)):
                logger.error(
                    "⚠️ 内部升级收件人与客户邮箱相同，已跳过发送工单，请检查产品 owner / fallback 配置"
                )
                return False
    ...
    if is_red_alert:
        ...
        if calm_intense:
            reason_internal = f"{reason_internal} [路由:情绪激烈→产品负责人链]"
        else:
